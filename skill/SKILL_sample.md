...
- **Python**: `C:/Users/deko/comfyui-gen/.venv/Scripts/python.exe`
- **スクリプト**: `C:/Users/deko/comfyui-gen/generate.py`
- **設定**: `C:/Users/deko/comfyui-gen/config.json`
- **LoRAマップ**: `C:/Users/deko/comfyui-gen/lora_map.json`
- **出力先**: `D:/_AI_Draw/Data/Images/Text2Img/`
- **前提条件**: ComfyUIがStability Matrixで起動済みであること
...
LoRA検索コマンド:
```bash
"C:/Users/deko/comfyui-gen/.venv/Scripts/python.exe" -c "
import json
with open('C:/Users/deko/comfyui-gen/lora_map.json', encoding='utf-8') as f:
    data = json.load(f)
...
- **デフォルト**: waiIllustriousSDXL_v150.safetensors（指定不要）
- **サブモデル**: `--checkpoint "prefectIllustriousXL_40.safetensors"`
- ユーザーが「別のモデルで」「illustriousで」「perfectで」等 → サブモデルに切り替え

### 6. 生成コマンド実行

```bash
"C:/Users/deko/comfyui-gen/.venv/Scripts/python.exe" "C:/Users/deko/comfyui-gen/generate.py" --prompt "masterpiece, best quality, absurdres, highres, PROMPT_HERE" --resolution RESOLUTION --steps STEPS --cfg CFG [OPTIONS] --json
```

**オプション一覧:**
- `--prompt "..."` — ポジティブプロンプト（必須）
- `--negative "..."` — ネガティブプロンプト（省略でデフォルト使用）
- `--resolution square|portrait|portrait_tall|landscape|landscape_wide`
- `--steps N` — ステップ数
- `--cfg N.N` — CFGスケール
...
- 1344x768 + hires 1.5x → 最終出力 2016x1152
